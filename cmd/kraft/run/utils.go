...
func (opts *Run) prepareRootfs(ctx context.Context, machine *machineapi.Machine) error {
	...
	ramfs, err := initrd.New(ctx, opts.Rootfs,
		...
		initrd.WithCacheDir(filepath.Join(opts.workdir, unikraft.BuildDir, "rootfs")),
	)
	if err != nil {
		return fmt.Errorf("could not prepare initramfs: %w", err)
	}

	initrdPath, err := ramfs.Build(ctx)
	if err != nil {
		return err
	}

	machine.Status.InitrdPath = initrdPath

	// Warn if the initrd path is greater than allocated memory
	fi, err := os.Stat(initrdPath)
	if err != nil {
		return err
	}
